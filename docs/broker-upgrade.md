...
worker grant to include `hardware`, `recovery` and `rotation`. It runs offline on
Linux/macOS. It preserves all six service identities, listener addresses, TLS
...
revision `5083e68c8f76` and the current `87aa1bdf56ea` renderer. The previous
initializer distribution is revision
`938ea1a77eba4c9eb4517db629fc42ef2f8b96ad`. Unknown/custom permission changes,
...
`change_required` and `added_worker_requests`. Review the three additional worker
requests and retain the exact `before_sha256`. No seed or configuration contents
are printed.
...
| `broker-upgrade-v1.json` | Immutable before/after hash journal |
| `broker-before-v1.json` | Exact original configuration; retained after success |
| `broker-next-v1.json` | Fully written target staging file; consumed by publication |
| `broker-upgrade-v1.complete.json` | Completion marker after publication and directory sync |
...
then starts with the upgraded configuration. The fixture verifies all three new
subscriptions, continued rejection of a broader wildcard, unchanged service
seeds, the retained message/consumer and subsequent command delivery into the
same pending consumer. It requires an explicit passing process test; missing
binary inputs cannot silently skip it.

The native Linux amd64/arm64 workflow pins both historical source revisions and
