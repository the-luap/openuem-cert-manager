...
IP addresses, URL paths and ports are rejected. Console, broker and optional
database names must be distinct. Use these same names in private service URLs and Docker
...
`--database-dns` is optional and has no default. Set it when creating a fresh PKI
that will also serve PostgreSQL. Omitting it preserves the original backend-only
configuration format and exported files. Database names are bound with the other
...
For PostgreSQL, use `database/server.pem` and `database/server.key` as its
`ssl_cert_file` and `ssl_key_file`, with TLS enabled. The database server name in
the application and administrator URLs must match one of the configured database
DNS names. Mount only the public `trust/backend-ca.pem` into database clients and
select it with `sslrootcert` and `sslmode=verify-full`. The database leaf permits
server authentication only and uses a separate P-256 PKCS#8 key. Neither database
passwords nor client certificates are generated by this option. Account creation
uses the console's separate protected database credential/bootstrap commands.
PostgreSQL must be able to read its private key under the chosen runtime UID with
private permissions; this initializer does not change service mount ownership.
...
server authentication only, as does the optional database leaf. The CA cannot
issue subordinate CAs. Authority, gateway, broker and database keys use P-256;
the console key uses RSA-3072 for its existing
...
Database tests also verify distinct service keys and DNS names, server-only
usage, successful hostname-verified TLS, original backend-only state compatibility,
rejected mode/name changes, interruption after all fourteen database-enabled export
steps and refusal to recreate missing committed database material. The console's
isolated PostgreSQL suite uses the actual private PKI executable together with its
